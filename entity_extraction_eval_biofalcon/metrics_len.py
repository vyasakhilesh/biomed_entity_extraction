...
def main():
    ...
    with open(path_output+'value_ent_score.pkl','rb') as f:
        value_arr = pickle.load(f)

    with open(path_output+'value_abbr_ent_score.pkl','rb') as f:
        value_abbr_arr = pickle.load(f)
    ...
    df_train['value_cui_score'] = value_arr
    df_train['value_abbr_cui_score'] = value_abbr_arr
    ...
    df_train['value_cui_len'] = df_train['value_cui_score'].apply(lambda x: sum([len(element) for element in x]))
    df_train['value_abbr_cui_len'] = df_train['value_abbr_cui_score'].apply(lambda x: sum([len(element) for element in x]))
    ...
    df_train['cui_jacD_mev_value'] = jaccard_arr_flat(mev_arr, value_arr)
    df_train['cui_jacD_mev_value_abbr'] = jaccard_arr_flat(mev_arr, value_abbr_arr)
    ...
    # overlap coefficient
    df_train['cui_OC_mev_value'] = oc_arr_flat(mev_arr, value_arr)
    df_train['cui_OC_mev_value_abbr'] = oc_arr_flat(mev_arr, value_arr)
