...
def main():
    ...
    df_pretrained_cui_all.to_csv(path_output+'cui_data_lsh.csv', index=False, header=False)
     
    datafile = path_output+"cui_data_lsh.csv"
    ...
    lsh.write_clusters_to_file( merged_similarity_groups, path_output+"clusters_100.txt" )
    
    with open(path_output+'clusters_CN_100.txt', 'w') as fw:
        with open(path_output+'clusters_100.txt', 'r') as f:
            lines = []
            for line in f:
